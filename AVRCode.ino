    ...
    uint32_t current_tick = get_millis();
        ...
        if (current_tick - last_tick >= 40) {
             last_tick = current_tick;
              ...
              final_ultrasonic_reading = filtered_arr[3];
...
float calculatePID(float distance) {
  ...
  // 1. Wider Deadzone for Stability
  if (error < 0.25 && error > -0.25) { 
      error = 0; 
      // Optional: integral *= 0.95; // Slowly bleed off integral at center
  }

  // 2. Only accumulate integral if we are OUTSIDE the deadzone
  if (error != 0) {
      integral += error * dt_fixed;
  }
  
  // Constrain Integral (Anti-windup)
